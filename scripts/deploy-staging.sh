#!/usr/bin/env bash
# =============================================================================
# PaperlessMD — Staging Deploy Script
# Pulls from the `staging` branch instead of `main`.
#
# Usage (manual):
#   bash /usr/local/bin/paperlessmd-deploy-staging
# =============================================================================
set -euo pipefail
...
echo "" >> "$LOG"
echo "===== STAGING Deploy started: $(date '+%Y-%m-%d %H:%M:%S') =====" >> "$LOG"

# ── 1. Pull latest code from staging branch ───────────────────────────────────
cd "$APP_DIR"
git fetch origin staging 2>> "$LOG"
git reset --hard origin/staging 2>> "$LOG"
echo "Git pull (staging): OK" >> "$LOG"
...
echo "===== STAGING Deploy finished: $(date '+%Y-%m-%d %H:%M:%S') =====" >> "$LOG"
echo "Staging Deploy OK — $(date '+%Y-%m-%d %H:%M:%S')"
