<?php
/**
 * Migration: add 'provider' to staff.role ENUM
 *
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifyCsrf($_POST['csrf_token'] ?? '')) {
    try {
        $pdo->exec("ALTER TABLE staff MODIFY COLUMN role ENUM('admin','ma','billing','scheduler','provider') NOT NULL DEFAULT 'ma'");
        $message = 'Migration complete. The <code>provider</code> role is now available.';
        $success = true;
...
$pageTitle  = 'Migration: Provider Role';
$activeNav  = '';
...
<div class="max-w-xl mx-auto mt-8">
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
        <h2 class="text-xl font-extrabold text-slate-800 mb-1">Provider Role Migration</h2>
        <p class="text-slate-500 text-sm mb-6">Adds <code class="bg-slate-100 px-1 rounded">provider</code> to the <code class="bg-slate-100 px-1 rounded">staff.role</code> ENUM column.</p>
