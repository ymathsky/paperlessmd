...
// If saved provider name is blank, auto-fill from today's schedule or patient's assigned provider
if (!$_provSavedName) {
    try {
        ...
        $_provSavedName = (string)($__sp->fetchColumn() ?: '');
        ...
        if (!$_provSavedName) {
            ...
            $_provSavedName = (string)($__sp2->fetchColumn() ?: '');
        }
        ...
        if (!$_provSavedName) {
            ...
            $_provSavedName = (string)($__pat->fetchColumn() ?: '');
        }
...
$formMeta = [
    ...
    'new_patient_pocket'     => ['label' => 'New Patient Packet',                 'icon' => 'bi-folder2-open',             'color' => 'text-indigo-700', 'bg' => 'from-indigo-700 to-indigo-600'],
];
...
<style>
  ...
  .bwc-header          { display: flex; flex-direction: row; align-items: center; justify-content: center; gap: 14pt; margin-bottom: 10pt; border-bottom: 1.5pt solid #000; padding-bottom: 8pt; }
  .bwc-header-logo     { width: 60pt; height: 60pt; object-fit: contain; flex-shrink: 0; }
  .bwc-header-text     { text-align: left; }
